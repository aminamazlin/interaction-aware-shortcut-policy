...
except ModuleNotFoundError:  # pragma: no cover - cluster env ships gymnasium only
...
class AsyncVectorEnv(VectorEnv):
    ...
    def reset_wait(self, timeout=None, seed=None, options=None):
        ...
        if not self.shared_memory:
            self.observations = _stack_obs(results)
    ...
    def step_wait(self, timeout=None):
        ...
        if not self.shared_memory:
            self.observations = _stack_obs(observations_list)
...
def _stack_obs(obs_list):
    """Stack a list of per-env observations into a batched structure.

    Works for both plain numpy arrays and dicts of numpy arrays, without
    relying on gym's `concatenate` (whose signature changed across versions).
    """
